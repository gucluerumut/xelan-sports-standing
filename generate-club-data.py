...
# Define clubs for each league with their Instagram usernames
leagues_config = {
    'Premier League': {
        'country': 'England',
        'slug': 'premier-league',
        'clubs': [
            ...
            ('lcfc', 'Leicester City'),
            ('nffc', 'Nottingham Forest'),  # Not in data, will use fallback
        ]
    },
    'La Liga': {
        'country': 'Spain',
        ...
        'clubs': [
            ...
            ('caosasuna', 'CA Osasuna'),  # Real account
...
output = []
output.append("// Real Instagram data for Xelan Sports Standing")
...
for league_name, league_config in leagues_config.items():
    ...
    for username, display_name in league_config['clubs']:
        if username in username_map:
            ...
            # Estimate engagement based on followers (rough approximation)
            ...
            output.append(f"      tiktokFollowers: {int(followers * 0.3)},  // Estimated")
            output.append(f"      tiktokViews: {int(followers * 50)},  // Estimated")
            output.append(f"      twitterFollowers: {int(followers * 0.4)},  // Estimated")
            output.append(f"      twitterEngagement: {engagement_rate * 0.8:.1f},  // Estimated")
            output.append("    },")
            output.append("    digitalScore: 0,  // Will be calculated")
            output.append("  },")
            club_id += 1
        else:
            print(f"❌ Missing: {username} ({display_name})")
...
print("\n✅ Generated lib/club-data-real.ts with real Instagram data!")
